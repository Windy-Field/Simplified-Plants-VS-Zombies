...
这是一款使用 Java (Swing) 编写的植物大战僵尸小游戏，目前包含了冒险模式的基础玩法，还有传送带和坚果保龄球两种特殊关卡。
...
- 目前一共 6 个关卡（第 1~6 关），通关后自动进入下一关。
- 经典关卡开始前需从 17 张候选卡中挑 8 张放入卡槽。
- 已完成阳光掉落与收集动画、卡片冷却、小推车防线等基础机制。
- 传送带关卡（卡片随机送出，不花阳光）和坚果保龄球关卡。
- 右上角设置了 1x / 2x / 3x 加速按钮。
- **GUI 关卡编辑器：拖动僵尸编排每一波出怪，调节初始阳光、阳光生成速度和出怪速度，还能指定某格随机行出怪、禁用或必选某些植物。**

## 运行环境

- 最好是 Windows 系统（启动脚本基于 PowerShell），其余系统不保证能正常运行。
- JDK 17+（通过 IntelliJ IDEA / Maven 运行时可能需要 JDK 21）
